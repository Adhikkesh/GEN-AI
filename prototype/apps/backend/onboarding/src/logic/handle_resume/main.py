...
RESUME_BUCKET_NAME = "your-project-id-resumes" 
...
def handle_resume(request):
    ...
    try:
        ...
        filename = data.get('filename') # You can also use 'fileName'
        if not filename:
    ...
    try:
        # 3. Get resume from GCS
        print(f"--- DEBUG: Getting resume {filename} for user {user_id}")
        bucket = storage_client.bucket(RESUME_BUCKET_NAME)
        blob_name = f"{user_id}/{filename}"
        blob = bucket.blob(blob_name)

        if not blob.exists():
            return f"File not found: {blob_name}", 404

        resume_bytes = blob.download_as_bytes()
        resume_mime_type = "application/pdf" if filename.lower().endswith('.pdf') else "image/png"
        resume_part = Part.from_data(data=resume_bytes, mime_type=resume_mime_type)

        # 4. STEP 1: Extract Skills
        skill_prompt = "You are an expert resume parser. Extract all technical and soft skills. Respond *only* in this exact JSON format: {\"skills\": [\"Skill 1\", \"Skill 2\"]}"
        ...
        # 6. STEP 3: Get Roadmaps (in a loop)
        for rec in analysis_data.get("recommendations", []):
            roadmap = _get_roadmap(rec.get("career"), rec.get("skill_gaps"))
            rec["roadmap"] = roadmap

        # 7. STEP 4: Save EVERYTHING to Firestore
        final_data_to_save = {
            "skills": skills_list,
        ...
        print(f"--- DEBUG (SUCCESS): Full resume pipeline complete for {user_id}")

        del final_data_to_save["last_updated"]
